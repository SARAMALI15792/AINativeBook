...
# ---------------------------------------------------------------------------
# Module-level singleton — ensures middleware and dependency layer share one
# cache, one backoff counter, and one fetch lifecycle.
# ---------------------------------------------------------------------------
_shared_jwks_manager: Optional["JWKSManager"] = None


def get_shared_jwks_manager() -> "JWKSManager":
    """
    Return the process-wide singleton JWKSManager.

    Lazily initialises from application settings on first call.
    Both JWKSAuthMiddleware and get_current_user() dependency must use
    this function so they share a single JWKS cache.
    """
    global _shared_jwks_manager
    if _shared_jwks_manager is None:
        from src.config.settings import get_settings
        settings = get_settings()
        _shared_jwks_manager = JWKSManager(
            jwks_url=settings.better_auth_jwks_url,
            cache_ttl_minutes=5,
        )
        logger.debug("Shared JWKSManager singleton initialised")
    return _shared_jwks_manager
