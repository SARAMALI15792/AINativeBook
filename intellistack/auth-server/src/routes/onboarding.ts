 ...
 * Save all personalization preferences at once and mark onboarding as complete.
 * Accepts any preferences object — no step-by-step validation required.
 */
router.post('/preferences', async (req, res) => {
  try {
    ...
    if (!preferences || typeof preferences !== 'object' || Array.isArray(preferences)) {
      return res.status(400).json({ error: 'VALIDATION_ERROR', message: 'preferences must be a non-null object' });
    }

    await db
      .update(userTable)
      .set({ preferences, onboarding_completed: true })
      .where(eq(userTable.id, session.user.id));
    ...
    return res.status(500).json({ error: 'SERVER_ERROR', message: 'Failed to save preferences' });
  }
});
