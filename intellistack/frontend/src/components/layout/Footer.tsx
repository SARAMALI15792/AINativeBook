...
export function Footer({ className = '' }: FooterProps) {
  ...
  const footerLinks = {
    learning: [
      ...
      { label: 'Stage 5: Capstone',        href: `${docusaurusUrl}/stage-5/intro`, external: true },
      { label: 'Dashboard', href: '/dashboard' },
    ],
    resources: [
      { label: 'Documentation', href: docusaurusUrl, external: true },
  ...
  const linkCls =
    'text-text-tertiary hover:text-accent-blue transition-all duration-normal hover:translate-x-1 inline-block text-sm';

  return (
    <footer className={`bg-bg-secondary relative ${className}`}>
      {/* Gradient top border */}
      <div className="h-px bg-gradient-to-r from-accent-blue via-accent-violet to-transparent" />
      ...
      <div className="container mx-auto px-4 py-12 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-10">
          <div className="flex flex-col gap-4">
            {/* IS logo mark + brand name */}
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-accent-blue to-accent-violet flex items-center justify-center shadow-glow-blue">
            ...
            <p className="text-text-tertiary text-sm max-w-xs">
              AI-Native Learning for Physical AI &amp; Robotics
            </p>
            {/* Social links */}
            <div className="flex space-x-4 mt-1">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="text-text-tertiary hover:text-accent-blue transition-all duration-normal hover:scale-110 hover:shadow-glow-blue rounded-full p-1"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {/* Learning */}
            <div>
              <h3 className="text-text-primary font-semibold mb-4 text-sm uppercase tracking-wider">Learning</h3>
        ...
        {/* Bottom bar */}
        <div className="pt-8 border-t border-border-subtle flex flex-col md:flex-row justify-between items-center gap-2">
          <p className="text-text-tertiary text-sm">
            © {new Date().getFullYear()} IntelliStack. All rights reserved.
          </p>
          <p className="text-text-tertiary text-sm">
            🤖 Built for Physical AI Education
          </p>
        </div>
      </div>
    </footer>
