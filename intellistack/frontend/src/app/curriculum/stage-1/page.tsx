  ...
  useEffect(() => {
    const loadStageData = async () => {
      try {
        ...
        if (!user) {
          router.push('/auth/login?redirect=/curriculum/stage-1');
          return;
        }
        ...
        // Fetch stage data
        const stageResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/learning/stages/stage-1`, {
          headers: authHeaders,
          credentials: 'include',
        });
        if (!stageResponse.ok) {
        ...
        const contentResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/learning/stages/stage-1/content`, {
          headers: authHeaders,
          credentials: 'include',
        });
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <button
                    ...
                    onClick={() => router.push(`/curriculum/stage-1/${item.slug}`)}
                  >
                    View content
